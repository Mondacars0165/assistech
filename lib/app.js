...
app.post('/register', (req, res) => {
  ...
  bcrypt.hash(password, saltRounds, (err, hash) => {
    ...
    const userValues = [rut, hash, rol_id];
    ...
    db.query(userQuery, userValues, (err, results) => {
      if (err) {
        ...
        console.log('rol_id received:', rol_id);
        res.status(200).json({ success: true, message: 'Usuario registrado con éxito', userId: results.insertId });
      }
    });
  });
});
